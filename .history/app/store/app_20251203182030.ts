...
export const useAppStore = defineStore("app", {
  ...
  actions: {
    ...
    async getWebSettings() {
      ...
      await $api.get("web_settings").then((res) => {
        ...
        for (const [key, value] of Object.entries(res.data.data)) {
          const objItem: any = {}
          (value as any[]).map((item: any) => objItem[item.key] = item.value)
          obj[key] = objItem
        }
        ...
        if (!useCookie("changeLocale").value) {
          const defaultLang = obj.website_setting?.website_default_language ?? 'en';
          useCookie("changeLocale").value = defaultLang;
          i18n.setLocale(defaultLang as "en" | "ar");
        }
    ...
    async setDefultColor() {
      ...
      let colorsToProcess: { [key: string]: string } = {};

      if (Array.isArray(this.settingsData.website_colors)) {
        // If it's an array, convert to object
        this.settingsData.website_colors.forEach((color: any) => {
          colorsToProcess[color.key] = color.value;
        });
      } else if (this.settingsData.website_colors && typeof this.settingsData.website_colors === 'object') {
        // If it's already an object, use it directly
        colorsToProcess = this.settingsData.website_colors as { [key: string]: string };
      } else {
        // Fallback to default colors
        colorsToProcess = this.colors;
      }

      // Apply colors
      for (const [key, value] of Object.entries(colorsToProcess)) {
        document.documentElement.style.setProperty(
          `--${key}`,
          value
        );
        ...
        if (key === "website_success_color") {
          document.body.style.setProperty(`--toast-background-color`, value);
          document.body.style.setProperty(`--v-theme-success`, value);
          document.body.style.setProperty(`--v-theme-on-success`, value);
          document.body.style.setProperty(`--toastify-color-success`, value);
          document.body.style.setProperty(
            `--toastify-color-progress-success`,
            value
          );
        ...
        if (key === "website_warning_color") {
          document.body.style.setProperty(`--v-theme-error`, value);
          document.body.style.setProperty(`--toastify-color-error`, value);
          document.body.style.setProperty(
            `--toastify-color-progress-error`,
            value
          );

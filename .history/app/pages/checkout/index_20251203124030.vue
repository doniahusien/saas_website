  ...
  <div class="min-h-screen bg-semi-white w-full flex justify-center py-8">
    <div class="w-full mx-auto p-8 grid grid-cols-1 lg:grid-cols-5 gap-10">
      ...
      <div class="lg:col-span-2 space-y-5 p-5 rounded-3xl bg-white">
        ...
        <div v-if="availableWallet > 0" class="mt-3 flex items-center gap-3">
          <input
            type="checkbox"
            id="use-wallet"
            v-model="useWallet"
          ...
          <label
            ...
            class="flex items-center gap-2 cursor-pointer select-none"
          >
            <span class="text-base font-medium">{{$t('LABELS.Use wallet balance')}}</span>
            <span
              class="px-3 py-1 text-sm font-bold text-white bg-linear-to-r from-green-500 to-emerald-600 rounded-full"
            >
              {{ availableWallet }} {{ currency }}
            </span>
          </label>
        </div>
      ...
      <teleport to="body">
        <general-modal
          v-if="openDeliveryAddressModal"
          @close="openDeliveryAddressModal = false"
          :persist="true"
...
const confirmOrder = async (values) => {
  ...
  try {
    ...
    form.append("has_wallet", useWallet.value && wallet > 0 ? "1" : "0");

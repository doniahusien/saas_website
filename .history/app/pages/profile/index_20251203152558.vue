<template>
  <div class="container mx-auto space-y-16 px-5 md:px-0 my-20">
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-5 gap-8 justify-content-between">
        ...
        <div class="lg:col-span-3 flex flex-col gap-5">
          ...
          />
          <button class="text-btn ms-auto text-xl font-bold">{{ $t("TITLES.changePhone") }}</button>
          <inputsBaseInput
            id="email"
            name="email"
            placeholder=" "
    ...
    <teleport to="body">
      <div class="relative">
        <general-modal
          classes=" !w-full md:!max-w-[600px] bg-white"
          :persist="true"
          v-if="openModal"
...
const passwordSchema = yup.object().shape({
  ...
  password_confirmation: yup
    .string()
    .oneOf([yup.ref("new_password")], t("ERRORS.passwordsMustMatch"))
...
function changePassword(values) {
  ...
  frmData.append("old_password", values.current_password);
  frmData.append("password", values.new_password);

...
const paymentLabel = computed(() => {
  ...
  // Prefer first non-points method, otherwise first entry
  const entry: any =
    list.find((e) => e && !("points" in e)) ?? list[0];

  if (!entry) return t("TITLES.Cash");

  // Handle shape { type: 'cash' } if backend ever sends it
  let method: string | undefined;
  if (typeof entry === "object" && "type" in entry) {
    method = String((entry as any).type).toLowerCase();
  } else {
    const keys = Object.keys(entry);
    method = keys[0]?.toLowerCase();
  }
  ...
  switch (method) {
    ...
    default:
      return (method || "cash").charAt(0).toUpperCase() + (method || "cash").slice(1);
  }
});
...
const paymentIcon = computed(() => {
  ...
  if (!entry) return "icon-park-outline:wallet";

      ...
      <ul class="profile-menu branch-list overflow-y-auto space-y-7">  
        <template v-for="(item, indx) in profileList" :key="indx">
          <li v-if="item.path">
            ...
            >
              <div class="flex size-14 items-center justify-center rounded-full bg-light-gray p-3">
       ...
       <!--  <li v-if="item.name == t('LABELS.notifications')">
          <button type="button">
            <div
              class="flex h-[55px] w-[55px] items-center justify-center rounded-full bg-secondary p-3"
            >
              <div class="strokeBlack">
                <nuxt-icon :name="item.icon" class="text-3xl text-text" />
              </div>
            </div>
              ...
              <USwitch
                ...
                :class="
                  userData.notifiable
                    ...
                    : 'bg-gray-200 bg-third'
                "
                class="relative inline-flex !h-7 !w-12 items-center rounded-full"
              >
                <span
                  :class="
                    userData.notifiable ? 'translate-x-6' : 'translate-x-1'
                  "
                  class="inline-block h-5 w-5 transform rounded-full bg-website_white transition"
                  v-if="$i18n.locale == 'en'"
                />
                <span
                  :class="
 ...
 -->
          <li v-if="item.name === t('LABELS.loyalty Card')">
            ...
            >
              ...
              <div class="ms-auto bg-[#e9f4ec] px-3 py-1 rounded-full text-success flex items-center gap-1">

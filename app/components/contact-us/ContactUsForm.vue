  ...
  <div class="container mx-auto mt-5 rounded-lg bg-semi-white pe-0">
    <div class="grid grid-cols-1 gap-10 lg:grid-cols-2">
      <div class="flex flex-col p-5 items-start justify-center space-y-8">
        <h2
          class=" text-4xl font-bold capitalize lg:text-6xl"
        >
          {{ $t("TITLES.contact us") }}
        </h2>
        <p
        ...
        >
          <InputsBaseInput
            ...
            type="text"
          />
          <InputsPhoneInput codeName="phone_code" phoneName="phone" :placeholder="$t('LABELS.phone')" />
          <InputsSelect
            id="message_type"
            name="message_type"
          ...
          <button
            :disabled="loading"
            type="submit"
            class="base-btn ms-auto mt-7 w-fit px-12 py-5"
          >
            {{ $t("BUTTONS.send message") }}
            <Icon :name="$i18n.locale == 'en' ? 'mdi:arrow-right' : 'mdi:arrow-left'" class="size-6" />
          </button>
        </VeeForm>
      </div>
      <div class="relative ">
        <div
          class="absolute -top-9 end-1 z-30"
          ...
          <div
            ...
            >
              ...
              <div v-if="appStore.settingsData" class="space-y-1">
                <div
                  v-for="phone in appStore.settingsData.contact_us
                    ?.phone_number"
                  :key="phone.id"
                >
                  <a
...
const schema = yup.object().shape({
  ...
  phone: yup
    ...
    .nullable(),
  phone_code: yup.string().required(t("ERRORS.isRequired", { name: t("LABELS.phoneCode") })),
  message_type: yup
    .string()
